...
const lambdaFunctions = ['create_memo', 'delete_memo', 'get_memo', 'list_memos', 'update_memo'];
  ...
  build: {
    ...
    rollupOptions: {
      input: {
        // Lambda関数のエントリーポイント
        ...Object.fromEntries(
          lambdaFunctions.map((fn) => [fn, resolve(__dirname, `${fn}/index.ts`)]),
        ),
      ...
      output: {
        ...
        // AWS SDKは外部化（Lambda実行環境に含まれているため）
        banner: `
