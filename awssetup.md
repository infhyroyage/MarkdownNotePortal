...
### 4. Amazon Bedrock のシステム定義の推論プロファイル ID ・ファウンデーションモデル ID の取得

[POST] /formatのREST APIでAmazon Bedrockの`InvokeModel` API を呼び出す際に必要な、クロスリージョンルーティング用に AWS があらかじめ提供しているシステム定義の推論プロファイル ID と、その推論プロファイルがルーティングするファウンデーションモデル ID を取得する。
...
3. 「システム定義」タブの表に表示されるモデルのうち、利用したいモデルを1つ選択する。
4. 選択したモデルの「推論プロファイル ID」の列の値(`global.anthropic.claude-haiku-4-5-20251001-v1:0`など)を控える。
5. 選択したモデルの「モデル」の列にあるファウンデーションモデルのリンクを押下し、リンク先に記載されているモデルIDの値(`anthropic.claude-haiku-4-5-20251001-v1:0`など)を控える。
   - 推論プロファイル ID から、先頭のリージョンプレフィックス `global.` / `us.` / `apac.` などを取り除いた値と一致する。
...
| 変数名                       | 変数値                                                       |
| ---------------------------- | ------------------------------------------------------------ |
| BEDROCK_FOUNDATION_MODEL_ID  | Amazon Bedrock のファウンデーションモデル ID                 |
