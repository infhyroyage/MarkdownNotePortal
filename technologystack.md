...
ヘッダー (`Header`) 右側のドロップダウンメニュー (`HeaderMenu`) に存在する "Export PDF" 項目を押下すると、選択中のメモの Markdown プレビュー表示内容を、 `WorkspacePreview` コンポーネントのDOMをキャプチャすることで、バックエンド API は使用せずに SPA のみで PDF 化し、ブラウザからダウンロードする。
この "Export PDF" 項目は、メモが選択されている場合のみ表示し、 PDF エクスポート中はメニュー項目を無効化してスピナーを表示する。
PDFファイル名は、ファイル名に使用できない文字(制御文字、`<>:"/\|?*` 等)を除去した `<選択中のメモのタイトル>.pdf` とし、除去した空になった場合は `memo.pdf` とする。
エクスポートする PDF の仕様は以下の通りである:

- 用紙サイズ: A4 縦 (`pt` 単位)
- 余白: 上下左右 40pt
- キャプチャ解像度: `scale: 2`
- 背景色: UI のライト/ダークテーマ設定にかかわらず、PDF は常にライトモードのレイアウトで出力
- 複数ページ: コンテンツが 1 ページを超える場合、画像の高さオフセットをずらしながら `addPage()` でページを追加

PDF エクスポートに使用するライブラリは以下の通りである:

- `html2canvas-pro`
  - プレビュー DOM を canvas 画像へ変換
  - Tailwind CSS v4 / daisyUI 5 が使用する `oklch()` カラー関数に対応するため、オリジナルの `html2canvas` ではなく本フォークを採用
- `jspdf`
  - canvas 画像を A4 縦向き PDF に貼り付け、ブラウザからダウンロード
