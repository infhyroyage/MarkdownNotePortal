...
- Node.js 22.x (バックエンドランタイム・TypeScript ランタイム)
...
- AWS Lambda 関数は、ESLint でコード品質を担保する。ESLint の静的解析は、以下のコマンドで実行する。
  ```bash
  npm run lint
  ```
...
- [ ] 以下のコマンドを実行して、すべてのユニットテストが成功し、カバレッジを 80% 以上にする:
  ...
  npm test
  ```
- [ ] 以下のコマンドを実行して、Lambda関数の ESLint の警告・エラーをすべて解消する:
  ```bash
  npm run lint
  ```
